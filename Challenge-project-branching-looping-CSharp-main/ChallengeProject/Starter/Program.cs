                ...
                if (ourAnimals[i, 0] != "ID #: ")       //if id is blank i.e. no id
                {
                    if (ourAnimals[i, 2] == "Age: ?")   //checks if age is blank or ?
                    {
                        do
                        {
                            ...
                            readResult = Console.ReadLine();
                            if (readResult != null)                 //only if readResult is not null
                            {
                                animalAge = readResult;
                                ourAnimals[i, 2] = "Age: " + animalAge;                 //assigns animalAge to array 
                                validEntry = int.TryParse(animalAge, out petAge);       //will only set numbers to true else everything will be false
                            }
                        } while (validEntry == false);
                    }
